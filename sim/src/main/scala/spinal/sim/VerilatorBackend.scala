...
std::chrono::duration<int> lastTimerSample;
...
JNIEXPORT jboolean API JNICALL ${jniPrefix}eval_1${uniqueId}
  (JNIEnv *, jobject, Wrapper_${uniqueId} *handle){
   handle->top.eval();
#ifdef COVERAGE
      auto timerTmp = high_resolution_clock::now(); 
      std::chrono::duration<double> diff = timerTmp - (handle->timerStart);
      auto diffCut = std::chrono::duration_cast<std::chrono::seconds>(diff);
   if (diffCut > lastTimerSample) {
      //cout << "Current time: " << diff.count() << ", Floored: " << diffCut.count() << endl;
      string path = "${new File(config.vcdPath).getAbsolutePath.replace("\\","\\\\")}/cov.dat" + to_string(diffCut.count());
      VerilatedCov::write(path.c_str());
      lastTimerSample = diffCut;
   }
#endif
...
//      case true =>  "-CFLAGS -DCOVERAGE --coverage"
      case true =>  "-CFLAGS -DCOVERAGE --coverage-line"
      case false => ""
    }
